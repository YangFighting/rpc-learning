...
Q: 为什么异步生成代理不需要设置代理类型？

- 同步生成代理方法`create`,需要根据入参的接口来代理出接口的实现类，代理实现类时使用JDK的反射方法来获取，通过代理实现类去调用服务提供者的方法，再调用服务提供者的方法会调用代理对象的invoke方法，这时候会Rpc请求协议，发送Rpc请求


- 异步生成代理方法`createAsync`直接将入参的接口和服务提供者的方法生成Rpc请求协议，发送Rpc请求
...
使用CGLib动态代理出接口的实现类，动态代理只会返回出接口，如何选择出接口的实现类是由spi（`ExtensionLoader.getExtension`）来得到代理工厂类`ProxyFactory`，这时候工厂类就已经表明是何种方式来实现代理，通过调用代理工厂类`ProxyFactory`的`getProxy`方法来获取服务提供者的实现类



## 第37章：基于SPI扩展JDK反射机制调用真实方法

服务提供者反射调用接口的真正实现是通过反射参数`reflectType`来选择不同的反射方式获取真实对象，本章采用spi的方式来获取真实对象

Q: 服务消费者的动态代理和服务提供者的反射机制有什么区别？
